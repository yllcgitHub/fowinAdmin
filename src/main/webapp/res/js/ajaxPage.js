...
(function($) {
	...
	function buildHeader(dataMapping) {
		...
		for (j = 0; j < dataMapping.length; j++) {
			...
			if (colObj.type == 'text' ||colObj.type == 'amount' || colObj.type == 'dict' || colObj.type == 'custom' || colObj.type == 'oper') {
	...
	function ajaxReqestData(curr) {
		...
		$.ajax({
			...
			success : function(data) {
				// if(curr>data.totalPage) curr=1;
				// debug(JSON.stringify(data));
				pageInit(data.totalPage, curr, data.totalRow);
