		...
		for(var i = 0; i < data.length; i++) {
			...
			if(data[i].children !== undefined && data[i].children.length > 0) {
				...
				for(var j = 0; j < data[i].children.length; j++) {
					ulHtml += '<dd>';
					var nodeHref =data[i].children[j].href; 
					var suffix = nodeHref.substring(nodeHref.indexOf("/fowinManage/"),nodeHref.length).replace("fowinManage/","");
//					var m = suffix.match(/^.+\/(\w+\.\w+)/i)
//					suffix = m[1];
					ulHtml += '<a href="javascript:;" data-url="' +serverDomain + suffix + '">';
				...
				var nodeHref = data[i].href;
			
				var suffix = nodeHref.substring(nodeHref.indexOf("/fowinManage/"),nodeHref.length).replace("fowinManage/","");
				var dataUrl = (data[i].href !== undefined && data[i].href !== '') ? 'data-url="' + serverDomain + suffix + '"' : '';
