...
layui.define(['element', 'common'], function(exports) {
	...
	Tab.prototype.closeAll = function() {
		...
		for(var i=0;i<globalTabIdIndex;i++) {
			element.tabDelete(ELEM.tabFilter, 1);
		}
		globalTabIdIndex=0;
	};
	/**
	 ...
	 * @param title
	 */
	Tab.prototype.closeOther = function(tabNo) {
		var that = this;
		var totalTabArr = ELEM.titleBox.find('li');
		var a = totalTabArr[tabNo];
		//console.log("not delete:tabNo:"+tabNo+";"+"Text:"+a.innerText);
		for(var i=0;i<totalTabArr.length;i++) {
			var indexLi = totalTabArr[i];
			//console.log("tabNo:"+tabNo+";i:"+i+";totallength:"+totalTabArr.length+"Text:"+indexLi.innerText);
			if (i!=0) {
				if (i<tabNo){
					element.tabDelete(ELEM.tabFilter, 1).init();
				}else if(i>tabNo){
					element.tabDelete(ELEM.tabFilter, 2).init();
				}
			}
	...
	Tab.prototype.tabAdd = function(data) {
		...
		if(tabIndex === -1) {
			...
			//切换到当前打开的选项卡
			tabIndex = ELEM.titleBox.find('li').length - 1;
			element.tabChange(ELEM.tabFilter, tabIndex);
		} else {
			element.tabChange(ELEM.tabFilter, tabIndex);
		}
		//var tabSize = ELEM.titleBox.find('li').length;
		var tabt = ELEM.titleBox.find('li').last();
		tabt.mousedown(function(e){
		    ...
		    if(e.button ==2){
				...
				var closeAllBtn= menu.find("#closeAll");
				closeAllBtn.unbind("click");
				closeAllBtn.click(function() {
					that.closeAll();
				});
				var closeOtherBtn = menu.find("#closeOther");
				closeOtherBtn.unbind("click");
				closeOtherBtn.click(function() {
					that.closeOther(tabNo);
				});
               }
		});
